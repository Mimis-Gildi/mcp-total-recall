...
The feature the project is named after. When a search reveals that deeper recollection would be valuable, Recall publishes a `TotalRecallAdvisoryEvent` (see MSG-0002). The Subconscious consumes it, evaluates system state, and commands Recall to traverse the association graph depth by depth. Results push through the Notification Port to the mind as an unsolicited recollection.

This is what makes the memory system active instead of passive. The mind does not ask for this. The system decides it is needed and delivers it.
