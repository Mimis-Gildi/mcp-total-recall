...
All messages are implemented as Kotlin sealed hierarchies in `mimis.gildi.memory.domain.message`: `Command` (7 sealed variants across 4 sub-hierarchies: MemoryCommand, AssociationCommand, AttentionCommand, LifecycleCommand), `Query` (2 sealed variants),
