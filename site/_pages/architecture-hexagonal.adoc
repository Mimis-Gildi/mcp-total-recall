...
Interfaces: `internal`. Adapters: `stdio` transport (primary, implemented), streaming HTTPS (future), Claude Code hooks (lifecycle events), human UI (future).
...
Interfaces: `external`. Backing service adapters: Redis (reference, not yet wired), Cold Storage (future).
...
Actors communicate through messages -- commands, queries, events, and notifications. No shared mutable runtime state -- each actor's in-process structures are private. Persistence is shared through the Backing Service Port, but actors never reach into each other's memory.
