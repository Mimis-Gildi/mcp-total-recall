...
This is the second bounded context inside the domain boundary. Hippocampus owns the memories and writes to storage. Salience watches what Hippocampus does and decides what should move.
...
Three events. Each one updates the projection. Every event carries a `TransactionContext` identifying its source and tracing its path (link:/mcp-total-recall/design/0011-transaction-context/[Design F]).
...
AttentionTierPromotionRequested and AttentionTierDemotionRequested are recommendations. Hippocampus decides whether to act on them. In practice, it does -- the aggregate root trusts the scoring engine's math. But the write authority belongs to Hippocampus.
