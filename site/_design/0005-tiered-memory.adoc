...
The Synapse bounded context manages typed relationships between memories. Those relationships need to be persisted somewhere.

Two options exist:

**Option A: Hippocampus owns it.** Associations are stored alongside memories through BackingServicePort. This preserves the single-writer invariant and keeps all persistent state in one place. But it muddies the aggregate root -- Memory is the root, not Association. And graph traversal patterns (find all associations of a memory, find paths between memories) are different from memory CRUD patterns.

**Option B: Synapse has its own storage port.** A separate outbound port for graph persistence. This respects bounded context ownership -- Synapse owns its own data. But it means two contexts write to storage, weakening the single-writer rule.

This question will be resolved in link:/mcp-total-recall/design/0007-association-graph/[Design E3: Synapse].
