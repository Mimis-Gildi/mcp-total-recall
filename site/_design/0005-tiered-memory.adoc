...
Hippocampus is the aggregate root. Every memory in Total Recall lives here. Every write to persistent storage goes through here. No other bounded context touches the data service port directly.

This is the first bounded context inside the domain boundary. The previous four documents described the boundary -- ports, adapters, ACLs, and the port topology. Now we're inside the kitchen.
...
Six memory commands in from three sources. Six events out to three consumers. One persistence channel through the ExternalDataServiceAdapter port. Everything Hippocampus touches, in one view.
...
The internal adapter (MemoryDataAdapter) translates `Memory` domain objects into DTOs and places them on the link:/mcp-total-recall/design/0004-port-topology/#port-3-externaldataserviceadapter-external[ExternalDataServiceAdapter port]. It picks up returned DTOs and translates back to domain objects when answering queries.

The port defines the DTO shape. The external adapter decides how to persist it. SQLite uses prepared statements and tables. Postgres would use different SQL. A file system would use directories and JSON. Hippocampus does not know which adapter is plugged in and does not care.

Two adapters can run simultaneously. SQLite for hot tiers (ACTIVE_CONTEXT, LONG_TERM). Cold storage for ARCHIVE and IDENTITY_CORE. Graceful shutdown depends on this -- the internal adapter flushes DTOs through the ExternalDataServiceAdapter port, and both external adapters persist their respective tiers.
...
Synapse is a dependent aggregate. It depends on the root aggregate's ID (Memory ID) to link associations to the memories they connect, but it manages its own data in its own file cabinet. Nobody from outside the domain boundary addresses Synapse directly -- only internal bounded contexts coordinate with it.
