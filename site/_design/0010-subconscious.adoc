...
Two inputs: lifecycle events from Cortex, and advisory events from the Memory Event Store. Multiple outputs to multiple targets. Salience gets decay sweeps. Hippocampus gets consolidation commands. All bounded contexts receive the ShutdownCommand for graceful convergence. Recall gets Total Recall traversal commands. NotificationPort gets break reminders and session prompts. No inbound port from the outside world. The mind does not talk to Subconscious directly.
...
When the shutdown is urgent -- the system is going down, resources are being reclaimed -- Subconscious sends `ShutdownCommand` to all bounded contexts. Each context handles its own shutdown responsibilities: Hippocampus flushes memory state to cold storage, Salience persists scores, Synapse persists associations. Hot-tier data in Redis gets persisted to durable storage. Working context that would normally evaporate at session end gets preserved instead.
