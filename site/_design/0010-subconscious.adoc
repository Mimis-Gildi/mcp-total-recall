...
sends break reminders when it has been too long, and drives Total Recall when the mind signals that search results were not enough. Nobody asks it to do these things.
It runs on timers, lifecycle events, and advisory signals, doing maintenance and deep work that the conscious mind should not have to think about.

This is the sixth and final bounded context inside the domain boundary. The other five respond to requests -- Cortex receives tool calls, Hippocampus stores memories,
Salience scores them, Synapse connects them, Recall assembles search results. Subconscious initiates. It watches the clock and acts when the time comes.
...
Two inputs: lifecycle events from Cortex, and advisory events from the Memory Event Store. Multiple outputs to multiple targets. Salience gets decay sweeps.
Hippocampus gets consolidation commands. All bounded contexts receive the ShutdownCommand for graceful convergence. Recall gets Total Recall traversal commands.
NotificationPort gets break reminders and session prompts. No inbound port from the outside world. The mind does not talk to Subconscious directly.
...
| TotalRecallAdvisoryRequestedEvent
...
Subconscious sends `ConsolidateMemoryCommand` to Hippocampus with a list of memory IDs and a merge strategy. Hippocampus merges them -- combining content, preserving metadata from all sources, maintaining the most protective tier. The original fragments are replaced by a single consolidated memory.
...
| SessionAuditPrompt
| The session is ending. Subconscious prompts the mind to reflect: what happened this session? Who matters enough to hold on to? Who do you refuse to lose? This is the claiming prompt -- the moment where passive storage becomes active identity.
|===
...
That is Total Recall. The mind searches (MSG-0002), evaluates the results, and decides whether deeper recollection is needed. If so, the mind signals through Cortex, which publishes a `TotalRecallAdvisoryRequestedEvent`. Subconscious consumes it from the Memory Event Store and decides whether to act.
...
When the shutdown is urgent -- the system is going down, resources are being reclaimed -- Subconscious sends `ShutdownCommand` to all bounded contexts. Each context handles its own shutdown responsibilities: Hippocampus flushes memory state to cold storage, Salience persists scores, Synapse persists associations. Hot-tier data gets flushed to durable storage. Working context that would normally evaporate at session end gets preserved instead.
...
Subconscious holds timer state, session tracking data, and scheduling rules in memory. All of it is transient. If Subconscious restarts, it rebuilds from the current session state (which Cortex provides via new SessionStarted events) and resumes its timers from defaults.
...
| Cortex
| Receives SessionStarted and SessionEnded lifecycle events. Lifecycle signals always come through Cortex.

| Salience
| Sends DecaySweep on a timer. Triggers the scoring engine to recalculate and identify tier changes.

| Hippocampus
| Sends ConsolidateMemoryCommand (background memory merging). ShutdownCommand also reaches Hippocampus as part of the all-context graceful shutdown.
...
| Watches
| SessionStarted and SessionEnded from Cortex. TotalRecallAdvisoryRequestedEvent from Memory Event Store.

| Schedules
| DecaySweep (to Salience), ConsolidateMemoryCommand (to Hippocampus), ShutdownCommand (to all bounded contexts), TotalRecallCommand (to Recall).
...
*Previous: link:/mcp-total-recall/design/0009-cortex/[Cortex]* -- the entry point where the outside world meets the domain boundary.
