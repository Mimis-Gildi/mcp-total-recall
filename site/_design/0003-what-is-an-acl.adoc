...
The ACL is a convenience construct -- a name for a pattern that exists named or not. Every restaurant has one. No restaurant built one on purpose.
...
The MCP Tool Handler does not know about `StoreMemoryCommand`. The domain handler does not know about JSON-RPC. They both know the shape of what crosses the MemoryPort -- because the core domain published that contract.
...
Hippocampus (cook -- Active Structure)
  finishes: processing StoreMemoryCommand
