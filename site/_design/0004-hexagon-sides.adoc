...
Four object shapes. Lifecycle is simpler than memory -- fewer things cross, but each one matters. A missed session end means the Daemon never triggers the audit prompt.
...
Session Context handler. Translates lifecycle events into session state. Loads identity and last session on start. Triggers Daemon on end.
...
| Memory (outbound)
| ID, content, metadata, tier, attention score, timestamps
...
Tiered Memory. The only bounded context that actually goes to storage. It puts Memory objects on the counter after processing commands. It picks up Memory objects from the counter when answering queries.
...
Two object shapes today. More will come as the Daemon's behavior is specified (context drift warnings, memory capacity alerts).
...
The Daemon. It decides when to notify based on timers, thresholds, and session state. It puts notification objects on the counter. It does not know whether the mind receiving the notification is Claude or a human.
...
*Next: the bounded contexts that live inside the hexagon, starting with link:/mcp-total-recall/design/0005-tiered-memory/[Tiered Memory] -- the aggregate root.*
