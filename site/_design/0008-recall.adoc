...
None of these sources know about each other. Hippocampus doesn't know about scores. Salience doesn't know about associations. Synapse doesn't know about search terms. Recall is the only place where all three come together.
...
Hippocampus handles writes -- StoreCommand, ClaimCommand, ReclassifyCommand. Those change state. Recall handles reads -- SearchQuery, ReflectQuery. Those don't change anything. They assemble from current state and return a view.
